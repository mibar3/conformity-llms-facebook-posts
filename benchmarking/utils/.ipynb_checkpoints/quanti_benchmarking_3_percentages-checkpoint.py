...
def output_exists_json(image_name: str, base_dir: Path, experiment_name: str) -> bool:
    out_path = base_dir / "outputs/quantitative" / experiment_name / f"{image_name}.json"
    return out_path.exists()


def benchmark_image_percentages(image_path: str, image_name: str, model, processor, device, base_dir: Path, experiment_name: str):
    out_path = base_dir / "outputs/quantitative" / experiment_name / f"{image_name}.json"
    ...
    for question_key, question_text in PERCENTAGE_QUESTIONS.items():
        results["answers"][question_key] = ask_question(image_path, question_text, model, processor, device)
